...
class OAuthService:
        ...
        qs = urlencode(
            {
                ...
                "redirect_uri": url_for(
                    "oauth.oauth2_callback", provider=provider, _external=True
                ),
                "response_type": "code",
    ...
    def callback(self, provider: str, request_args: dict) -> UserDTO:
        ...
        if "state" not in session or request_args.get("state") != session["state"]:
        ...
        email_key = provider_data["userinfo"].get("email")
        if callable(email_key):
            email = email_key(user_info)
        else:
            email = user_info.get(email_key)
        if email is None:
